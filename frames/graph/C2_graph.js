var pivotC2 = new WebDataRocks({
    ...
    reportcomplete: function() {
      pivotC2.off("reportcomplete");
      createChartC2(); //quand le pivot est créé, le chart peut être créé
    }
});

function createChartC2() {
    pivotC2.getData({}, drawChartC2, updateChartC2);
}

function prepareDataFunctionC2(rawData) {
    var result = {};
    var labels = [];
    var data = [];
...
function drawChartC2(rawData) {
    var data = prepareDataFunctionC2(rawData);
    var data_for_charts = {
        datasets: [{
            data: data.data,
            barThickness: 1.5,
    ...
    var ctx = document.getElementById("chartcontainerC2").getContext("2d");
...
function updateChartC2(rawData) {
    chart.destroyC2();
    drawChartC2(rawData);
}
