var pivotC2bis = new WebDataRocks({
    container: "#pivotC2bis",
    ...
    reportcomplete: function() {
      pivotC2bis.off("reportcomplete");
      createChartC2bis(); //quand le pivot est créé, le chart peut être créé
    }
});

function createChartC2bis() {
    pivotC2bis.getData({}, drawChartC2bis, updateChartC2bis);
}

function prepareDataFunctionC2bis(rawData) {
    var result = {};
    var labels = [];
    var data = [];
...
function drawChartC2bis(rawData) {
    var data = prepareDataFunctionC2bis(rawData);
    var data_for_charts = {
        datasets: [{
            data: data.data,
            barThickness: 1.5,
    ...
    var ctx = document.getElementById("chartcontainerC2bis").getContext("2d");
...
function updateChartC2bis(rawData) {
    chart.destroyC2bis();
    drawChartC2bis(rawData);
}
