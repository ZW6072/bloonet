var pivotC3 = new WebDataRocks({
    ...
    reportcomplete: function() {
      pivotC3.off("reportcomplete");
      createChartC3(); //quand le pivot est créé, le chart peut être créé
    }
});

function createChartC3() { //le chart est créé
        pivotC3.getData({}, drawChartC3, updateChartC3);
    };

function prepareDataFunctionC3(rawData) {
    var result = {};
    var labels = [];
    var data = [];
...
function drawChartC3(rawData) {
    var data = prepareDataFunctionC3(rawData);
    var data_for_charts = {
        datasets: [{
            data: data.data,
            borderWidth: 2,
    ...
    var ctx = document.getElementById("chartcontainerC3").getContext('2d');
...
function updateChartC3(rawData) {
    chart.destroyC3();
    drawChartC3(rawData);
};
